...
//
// This project is for the CFAP152152B0-0154:
//
//   https://www.crystalfontz.com/product/cfap152152b00154
//
...
// Include LUTs
#include "LUTs_for_CFAP152152B00154.h"
#include "Images_for_CFAP152152B00154.h"
...
void initEPD()
{
  ...
  // can be found on the CFAP152152B0-0154 datasheet on the Crystalfontz website	
  //-----------------------------------------------------------------------------
    ...
    if (0 == bmp_file.isDirectory())
    {
      ...
      if (0 != strstr(bmp_file.name(), ".BMP"))
      {
        ...
        if ((uint32_t)HRES*VRES * 3  + 50 <= size <= (uint32_t)HRES * VRES * 3 + 60)
        {
          ...
          static uint8_t one_line[HRES / divider * 3];
          ...
          for (int line = 0; line < VRES * divider; line++)
          {
            ...
            //send the line to the display
            send_pixels_Y(HRES / divider * 3, one_line);
          }
...
void loop()
{
  ...
  Load_Flash_Image_To_Display_RAM(HRES, VRES, Splash_Mono_1BPP, Splash_Yellow_1BPP);
